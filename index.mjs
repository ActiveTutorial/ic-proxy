...
export function pair(first, second) {
  ...
  const url = `https://neal.fun/api/infinite-craft/pair?ref=app&first=${encodeURIComponent(first)}&second=${encodeURIComponent(second)}`;
  return req.get(url, {
...
export function check(first, second, result) {
  ...
  const url = `https://neal.fun/api/infinite-craft/check?ref=app&first=${encodeURIComponent(first)}&second=${encodeURIComponent(second)}&result=${encodeURIComponent(result)}`;
  return req.get(url, {
    impersonate: "chrome136",
    userAgent: "M"
  })
